# Advisory Self-Review (2026-02-13T12:42:52.252817+00:00)
...
## Core Metrics
- Advisory rows: 1183
- Advisory trace coverage: 962/1183 (81.32%)
- Advice items emitted: 4835
- Engine events: 500
- Engine trace coverage: 251/500 (50.2%)
- Fallback share (delivered): 0.0%
- Strict action rate: 0.714
- Strict effectiveness rate: 0.972
- Trace mismatch count: 143
...
- `advisory-bench-baseline-20260213122850-e299d9-0018` | tool `Task` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0017` | tool `Bash` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0016` | tool `Task` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0015` | tool `Task` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0013` | tool `Read` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0012` | tool `Task` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0011` | tool `Task` | source `cognitive` | Constraint: in **exactly one state** at all times
- `advisory-bench-baseline-20260213122850-e299d9-0010` | tool `Bash` | source `cognitive` | Constraint: in **exactly one state** at all times
...
- Yes. Top repeated advisories account for ~61.21% of all advice items in this window.

## Top Repeated Advice (Noise Candidates)
- 582x (12.04%) Constraint: in **exactly one state** at all times
- 575x (11.89%) instead of this being a question let's make it better and say a collection evolution network with guardrails or should we say the guardrails?
- 566x (11.71%) lets push gitthub, and then clean up the whole system except this spark, and at some point we were thinking to bring the council of 50, as the agents can you se
- 464x (9.6%) "formatting": "ALWAYS put 'multiplier granted' on its own line with a line break before it. This makes it scannable when people scroll replies and creates
- 464x (9.6%) When using Bash, remember: Do it, and is this system fully connected to spark intelligence flow right now, for spark to e
- 308x (6.37%) Mac Mini + AI agent setup trending as "money printer" but requires TAO subnet participation for actual returns - not just local inference
