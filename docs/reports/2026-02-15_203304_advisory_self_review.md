# Advisory Self-Review (2026-02-15T20:33:04.229749+00:00)
...
## Core Metrics
- Advisory rows: 547
- Advisory trace coverage: 547/547 (100.0%)
- Advice items emitted: 1383
- Engine events: 500
- Engine trace coverage: 421/500 (84.2%)
...
- `advisory-bench-strict-20260215202959-2d80b1-0010` | tool `Bash` | source `cognitive` | Constraint: in **exactly one state** at all times
...
- 310x (22.42%) When using Bash, remember: Do it, and is this system fully connected to spark intelligence flow right now, for spark to e
- 307x (22.2%) lets push gitthub, and then clean up the whole system except this spark, and at some point we were thinking to bring the council of 50, as the agents can you se
- 229x (16.56%) Constraint: in **exactly one state** at all times
- 196x (14.17%) Always Read a file before Edit to verify current content
- 104x (7.52%) [Caution] I struggle with WebFetch fails with other tasks
